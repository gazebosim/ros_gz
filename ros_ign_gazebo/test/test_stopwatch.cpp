...
TEST_F(TestStopwatch, SimTime)
{
  ...
  ros_ign_gazebo::Stopwatch stopwatch;
  stopwatch.SetClock(clock);

  auto new_time = clock->now() + rclcpp::Duration(0, static_cast<uint32_t>(20e6));
...
void runTimer(ros_ign_gazebo::Stopwatch &_time)
{
  ...
  EXPECT_EQ(rclcpp::Duration(0, 0U),
